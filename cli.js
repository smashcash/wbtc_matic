...
/** Display WBTC account balance */
async function printETHBalance({ address, name }) {
  console.log(`${name} WBTC balance is`, web3.utils.fromWei(await web3.eth.getBalance(address)))
}
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  if (currency === 'WBTC' && refund !== '0') {
    throw new Error('The WBTC purchase is supposted to be 0 for WBTC withdrawals')
  }
  refund = toWei(refund)
  if (relayerURL) {
 ...
 */
async function init({ rpc, noteNetId, currency = 'wbtc', amount = '0.0001' }) {
  ...
  if (inBrowser) {
    ...
    try {
      if (web3 == undefined) {
        ...
        contractJson = await (await fetch('build/contracts/WBTCSmashnado.json')).json()
  ...
  erc20 = currency == 'wbtc' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  const instance = { currency: 'wbtc', amount: '0.0001' }
    //await init(instance)
    ...
    let senderBalance = web3.utils.fromWei(await erc20.methods.balanceOf(senderAccount).call(), 'gwei') * 10
    if ( parseFloat(senderBalance) < parseFloat(_amountStr) ) {
      w_display_notification('Insufficient WBTC Balance in your wallet to deposit.', 'error')
      return false;
    }
